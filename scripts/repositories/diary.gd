...
func find_by_date(date: Datetime) -> Array[Dictionary]:
	var lower_bound = date.get_the_beginning_of_the_day().to_unix_time()
	var upper_bound = date.get_the_end_of_the_day().to_unix_time()
	var diaries = super.select_rows("contentType='%s'" % ENTRY_CONTENT_TYPE)
	return diaries.filter(func (diary): return diary.content.date >= lower_bound and diary.content.date <= upper_bound)
