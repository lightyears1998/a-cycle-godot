...
var http_request := PromisedHTTPRequest.new()
...
func get_user_token(config: SyncServerConfig) -> String:
	Logcat.info(config.get_identifier() + " Requesting user token for user id %s." % config.user_id)
	...
	Logcat.info(config.get_identifier() + " Accquired user token %s for user id %s." % [token, config.user_id])
	config.token = token
	return token
